...
final class TrendingCollectionViewCell: UICollectionViewCell {
    ...
    func setData(dataModel: TrendingModel) {
        ...
        //MARK: set text
        self.lblDescription.text = dataModel.program?.programDetails?.title
        //MARK: set image
        self.imgView.image = nil
        self.activityIndicator.start(onView: self.imgView)
        self.imgView.sd_setImage(with: URL(string: dataModel.program?.programDetails?.thumbnail ?? ""), completed: { [weak self] _,_,_,_  in
